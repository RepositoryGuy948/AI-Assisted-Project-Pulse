<template>
  <div>
    <h1 class="text-h5 font-weight-bold mb-2">Invite Instructors</h1>
    <p class="text-body-2 text-medium-emphasis mb-6">
      Instructors will receive an email with a unique registration link.
    </p>
          ...
          <v-card-text>
            <v-form ref="formRef" validate-on="submit lazy">
              ...
              <v-textarea
                ...
                placeholder="professor@tcu.edu; client@company.com"
...
import { getSections, inviteInstructors } from '@/api'
...
const previewBody = computed(() => {
  const lines = [`Hello,`, ``, `${adminName.value} has invited you to join The Peer Evaluation Tool as an instructor.`]
  ...
  lines.push(
    ...
    'http://localhost:5173/register/instructor?token=<unique-token>',
    '',
...
async function sendInvitations() {
  ...
  sending.value = true
  try {
    const res = await inviteInstructors(form.value.sectionId, {
